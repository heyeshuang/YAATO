...
//opera没有得到测试
//todo:加入jquery UI，实现日期选择器（datepicker）/浮动窗口（dialog），但是还没有下定决心……
//todo:使生成内容可选
...
function main() {
	...
	$().ready(function() {
		...
		$("#outputCal").click(function() {
			...
			$vCalDiv.prepend($vCalArea);
			$vCalDiv.prepend('<h4>请在此处右击-全选-复制，粘贴到一个文本文件中，并将其扩展名改为.ics</h4>');
			$('div.window_neirong').prepend($vCalDiv);
